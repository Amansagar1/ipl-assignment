...
var addteamClicked = () => {
  window.open("../Html/addteam.html", "_self");
};

var AddplayerClicked = () => {
  window.open("../Html/Addplayer.html", "_self");
};
let detailofTeam = [];
let detailOfPlayer = [];
...
function currentLi(element) {
  ...
  icon.onclick = () => {
    window.open(`../Html/Teams.html?name=${element.textContent}`, "_self");
  };
  searchBar.classList.remove("active");
}
...
function makethisinclick(res) {
  ...
  window.open(`../Html/Teams.html?name=${clickedCard}`, "_self");
}
